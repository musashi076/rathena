...
int map_sql_init(void)
{
	...
	if (map_config.check_tables && !map_check_tables()) {
		ShowFatalError("map-server : A table is missing in sql-server, please fix it, see (sql-files/main.sql or sql-files/logs.sql for structure) \n");
		exit(EXIT_FAILURE);
	}
...
int log_sql_init(void)
{
	...
	if (!log_check_tables()) {
		ShowFatalError("map-server (log) : A table is missing in sql-server, please fix it, see (sql-files/logs.sql for structure) \n");
		exit(EXIT_FAILURE);
	}
#endif
	return 0;
